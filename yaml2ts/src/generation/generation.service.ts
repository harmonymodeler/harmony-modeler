...
generators[TYPESCRIPT_GENERATORS.jsonSchemaToTypeScript] =
...
export const generate = async (options: GenerateCommandOptions) => {
  ...
  if (options.debug) {
    ...
    logger.debug(
      `Using generator ${options.generator} of ${Object.keys(generators)}`
    );
  }

  const visited = new Promise((resolve, reject) => {
    var fileEmiter = walk(options.inputPath);
    fileEmiter.on("file", async (filePath) => {
      if (filePath.endsWith((".yaml"))) {
        const fileContent = fs.readFile(filePath, "utf-8");
        const jsonSchema = load(fileContent);
        const jsonSchemaPath = join(
          tempDir,
          filePath.replace(".yaml", ".generated.json")
        );
        if (options.debug) logger.debug(`Generated ${jsonSchemaPath}`);
        await fs.mkdir(dirname(jsonSchemaPath), { recursive: true });
        await fs.writeFile(jsonSchemaPath, JSON.stringify(jsonSchema));

        const outputFilePath = join(
          options.outputPath,
          basename(filePath.replace(".yaml", ".generated.ts"))
        );
        const typeScript =
          await generators[options.generator].generate(jsonSchemaPath);
        await fs.writeFile(outputFilePath, typeScript, { flag: "w+" });
        if (options.debug) logger.debug(`Generated ${outputFilePath}`);
        return outputFilePath;
      }
    });
    fileEmiter.on("end", (file) => {
      resolve("");
    });
  });
  ...
  return await Promise.all(promises).then((result) =>
    logger.log("Generation complete", result)
  );
};
